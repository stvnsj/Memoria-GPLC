% FILE: ~/GPLC/MEMORIA/sections/concreto.tex

\newpage
\section{Lenguaje Concreto e Intérprete}
...
\subsection{Sintaxis Concreta}\label{sec:concreto-sintaxis}
...
La operación de selección binaria probabilstica se denota en sintaxis
...
\subsection{Valores}\label{sec:concreto-valor}
...
\begin{gplcblock}[style=gplc-style,caption={choice 5.1},label={lst:return1}]
{
...
\subsection{Intérprete y Ejecución}

\paragraph{Intérprete.} El intérprete de GPLC es un programa
...
\paragraph{Ejemplos.} A continuación se presentan tres ejemplos que
...
probabilística, se ilustra en en el código \ref{lst:ex1}. Esta
...
\verb|false|. Sin embargo, el caracter semántico de la selección en
...
\lstinline|3|, ascritos por el tipo \lstinline|Real|, y ponderados por
una variables reales.
...
Elaborando sobre el código anterior anterior, se añade un
\lstinline|let| que liga la selección del código \ref{lst:ex1} a la
variable \lstinline|num|. Este ejemplo evidencia el caracter \emph{iterativo}
...
\begin{gplcblock}[caption={bloque \lstinline|let|},label={lst:ex2}]
let num = 0.25 $ 0 : 3 in
num + num
